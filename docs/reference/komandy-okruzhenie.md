...
Статический анализ — **PHPStan + Larastan**, конфиг в `phpstan.neon`. Уровень 7
держится без baseline и без `ignoreErrors`. Выше не поднято намеренно: на 8-м
остаются ошибки вида «`$request->user()` может быть null», хотя за middleware
`auth` он не бывает null, — их «починка» свелась бы к недостижимым веткам.
