...
Статический анализ — **PHPStan + Larastan**, конфиг в `phpstan.neon`. Уровень 9
держится без baseline и без `ignoreErrors`.
...
На 10-м остаётся 37 ошибок одного рода: данные из `validate()` приходят как
`mixed`, и каждое обращение к полю требует сужения.
