...
| Файл             | Назначение                                                                                     |
| ---------------- | ---------------------------------------------------------------------------------------------- |
| `map.ts`         | `makeMap(data)` → `GameMap`: тайлы, коллизии, зоны, `canHear`, `isSpotlight`, порталы, объекты |
| `scene.ts`       | `OfficeScene` — рендер карты, аватаров, реакций, порталов на PixiJS                            |
